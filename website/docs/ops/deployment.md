...
远程 MCP 同时提供 `list_projects`、`create_project`、`upload_source` 三个项目入口工具，以及 `open_draft`、`patch_draft`、`promote_draft`、`discard_draft` 四个草稿工具。草稿工具要求显式传入 `project`；`patch_draft` 与 `discard_draft` 还须传入 `open_draft` 或最近一次 `patch_draft` 返回的当前 revision，以防并发覆盖或误删。
