...
Remote MCP also provides three project entry tools, `list_projects`, `create_project`, and `upload_source`, plus four draft tools: `open_draft`, `patch_draft`, `promote_draft`, and `discard_draft`. Draft tools require an explicit `project`; `patch_draft`, `promote_draft`, and `discard_draft` also require the current revision returned by `open_draft` or the most recent `patch_draft` to prevent concurrent overwrites, stale promotion, or stale deletion.
